...
- 去掉上右边框（已通过全局配置实现）
- 使用统一的 COLORS 配色方案
- **色盲安全**：红绿不得做唯一区分——颜色必须叠加形状/线型/标记差异，灰度打印仍可区分
- **最小字号 ≥8pt**：轴标签/刻度/图例均不得小于
- **禁用双 y 轴**（左右刻度易误导；确需双量纲时拆双子图）
...
- 图例位置不遮挡数据
- 参考线标注（如基线、阈值）
- **绘图数值与源数据一致**：图形展现的数值必须来自实际计算结果，禁止示意性手画
...
**图表溯源登记**：每张图的数据特征输出末尾追加一行图题论断，格式：
`print(f"   [图题论断] 本图支撑的结论：<一句话，含关键数值>")`
——这行是论文手引用该图时不夸大的锚点（图题论断 = 图里数据实际能支撑的最大结论）。
